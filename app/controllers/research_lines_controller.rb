class ResearchLinesController < ApplicationController
  ...
  def create
    @research_line = ResearchLine.create(research_line_params)
  ...
  private
    ...
    def set_research_line
      @research_line = ResearchLine.searchById(params[:id])
    end

    # Never trust parameters from the scary internet, only allow the white list through.
    def research_line_params
      params.require(:research_line).permit(:name, :description)
    end
end
